...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- CÓDIGO PARA MANEJAR MODALES ---
    const viewModal = document.getElementById('viewModal');
    const editModal = document.getElementById('editModal');
    const deleteModal = document.getElementById('deleteModal');
    // Colección de todos los modales en esta página
    ...
    // Asignar eventos de cierre a todos los modales
    modals.forEach(modal => {
        if (modal) {
            // Asignar al botón 'X' (clase .close-btn)
            modal.querySelector('.close-btn').addEventListener('click', () => closeModal(modal));
            
            // Asignar al clic fuera del contenido (en el fondo)
    ...
    // --- LÓGICA ESPECIALIZADA PARA CADA BOTÓN DE ACCIÓN ---

    // 1. Lógica para botones "Ver Detalles"
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function (e) {
            e.preventDefault();
            // Llenar datos del modal de vista
            document.getElementById('viewNombre').textContent = this.dataset.nombre;
            document.getElementById('viewDni').textContent = this.dataset.dni;
            document.getElementById('viewEmail').textContent = this.dataset.email;
            document.getElementById('viewTelefono').textContent = this.dataset.telefono;
            document.getElementById('viewEspecialidad').textContent = this.dataset.especialidad;
            // Mostrar modal
            viewModal.classList.add('is-visible');
        });
    });

    // 2. Lógica para botones "Editar"
    ...
    // 3. Lógica para botones "Eliminar"
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function (e) {
            e.preventDefault();
            // Llenar datos del modal de eliminación
            document.getElementById('deleteNombre').textContent = this.dataset.nombre;
            const odontologoId = this.dataset.id;
            // Asignar el ID al link de confirmación
            document.getElementById('confirmDeleteBtn').href = `../odontologo?id=${odontologoId}`;
            // Mostrar modal
            deleteModal.classList.add('is-visible');
        });
    });

    // 4. Lógica para el botón "Cancelar" en el modal de eliminar
    // (Esta era la línea que estaba fuera de lugar y rompía todo)
